...
export default class PostService {
  ...
  async schedulePost(postIds: number[], publishAt: string) {
    try {
      return await PostTranslation.query()
        .whereIn('id', postIds)
        .update({ status: 'Scheduled', publishedAt: publishAt })
    } catch (error) {
      console.log(error)
  ...
  async updateScheduledPosts() {
    try {
      ...
      await Promise.all(
        scheduledPosts.map(async (postTranslation) => {
  ...
  async getPostById(id: number) {
    const post = await Post.query().where('id', id).preload('translations').first()
  ...
  async getAllPosts(filters: any = {}, page: number = 1, limit: number = 10) {
    const postTranslations = await PostTranslation.query()
      ...
      .if(filters.title, (query) => query.where('title', 'ILIKE', `%${filters.title}%`))
      .if(filters.publishedAfter, (query) =>
        query.where('publishedAt', '>=', filters.publishedAfter)
      )
      .if(filters.publishedBefore, (query) =>
        query.where('publishedAt', '<=', filters.publishedBefore)
      )
